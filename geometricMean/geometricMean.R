...
table <- read.csv("input/codon_tRNA_mapped_novo.txt", sep = "\t", stringsAsFactors = FALSE)
...
file <- file("output/EPI_tRNA_mapped_reads-geometricAverage.tsv", open = "a")

# Computes the number of rows at FASTA file.
numOfLines <- nrow(read.csv("input/Dm28c_CDS.fasta"))

# Reads the FASTA 2line file.
for (line in seq(from = 1, to = numOfLines, by = 2)) {
   fastaRecord <- read_FASTA_2line("input/Dm28c_CDS.fasta", line)
...
file <- file("output/MT_tRNA_mapped_reads-geometricAverage.tsv", open = "a")
